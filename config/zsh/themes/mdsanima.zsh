...
# Custom MDSANIMA configuration file for the `Oh My Zsh` and Powerlevel10k prompt shell terminal.
# Check this https://github.com/romkatv/powerlevel10k/ and https://github.com/ohmyzsh/ohmyzsh/ site
# for more inforamtion about this cool looking prompt.

# We have removed all unused commands and help comments from this configuration file.  Type in the
# terminal `p10k configure` to generate new config, but might be overwriting your current config.

# Generated by Powerlevel10k configuration wizard and customized by @mdsanima using the custom
# colors and emojis.  This themes is still work in progress.  Might be changed in the future.

# Based on romkatv/powerlevel10k/config/p10k-lean.zsh, checksum 26839.
# Wizard options: nerdfont-v3 + powerline, small icons, unicode, lean, 24h time, 1 line, compact,
# many icons, fluent, instant_prompt=verbose.  Config for Powerlevel10k with lean prompt style.

# Check the original configuration file thats is showed below for more information and custom one
# line print colormap command.
...
() {
  ...
  # Unset all config options. This allows you to apply config changes without restarting zsh.
